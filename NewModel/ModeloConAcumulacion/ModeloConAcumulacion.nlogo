extensions [csv]
globals[pubtotal total-energy total-papers total-distance-mean att-file probability]
turtles-own [
  energy
  pub
  mov
...
to export ;turtle proc
  export-all-plots (word prueba "/" numberAgents "-1-" initial-probability "-" probabilityIncrease "-curve.csv")
end
...
file-open (word prueba "/" numberAgents "-1-" initial-probability "-" probabilityIncrease "-turtles.csv")
end
...
ask turtles [
  ...
  if energy > 0 [
    if papers > 0 [
        ...
        set probability probability + probabilityIncrease
        set mov 1
        set pcolor black
    ]
  ]
...
to grow-papers
ask patches [
     if random (100 / (probability + initial-probability)) = 1 and papers = 0[
      set papers papers + 1
      set total-papers total-papers + 1
    ]
]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot probability"
