...
def createJoin(directory, table1,table2, input):
	...
	try:
		...
		if exception.errno != errno.EEXIST:
			raise
	tab1 = open(directory+'/averageResults/'+table1+input, 'r')
	tab2 = open(directory+'/testTables/'+table2+input, 'r')
	output = open(directory+'/testTables/'+table1+'-'+table2+'/'+input,'w')
	i=0
	for line in tab1:
	...
	i=0
	for line in tab2:
		output.write(listOfNumbers[i].replace("\n", '')+','+(line.split(',')[1]))
		i=i+1
	entrada.close()
	tab1.close()
	tab2.close()
...
def getAverageOfTablesOld():
	...
	movement = ["1"]
	initialProbability = ["0.01", "0.1", "1"]
	file=""
	for i in numberAgents:
		for j in movement:
			for l in initialProbability:
				file=i+"-"+j+"-"+l+"-turtles.csv"
